    ...
    var DirectionalLight = CSS3D.Object3D.extend({
        ...
        calc: function(object) {
            if (!(object instanceof CSS3D.Face)) {
                ...
                if (object.color === 'black' &&!object.useController) {
                    return;
                }
    ...
    var RubikCube = CSS3D.Object3D.extend({
        ...
        rotateCubes: function(target, dir) {
            ...
            if (type.slice(0, 6) === 'middle' ) {
             ...
             console.log(this._faceType, type, dir);
            $.ajax({
                method: 'POST',
                url: 'http://' + HOST + ':3000',
                data: {
...
(function(win, doc, Class, exports) {
    ...
    var Timer = Class.extend({
        ...
        clear: function() {
            this.time = 0;
            var el = this.el;
            el.innerHTML = this.parse(this.time);
        },
        stop: function() {
...
(function(win, doc, Matrix4, Class, CSS3D) {
    ...
    (function() {
        ...
        if (/demo2/i.test(query)) {
            ...
            rubikcube.translate([0, 0, -500]);
        }
        rubikcube.rotateX(-50);
        rubikcube.rotateY(40);
        rubikcube.random(-1); // stop random init cube -1
        ...
        jQuery.ajax({
            mathod: 'GET',
            url: 'http://' + HOST + ':3000/init'
        });

        var timer = new Timer();
        doc.querySelector('#recover').appendChild(timer.el);
        
        var recover_btn = doc.querySelector('#recover-btn');
        var init_btn = doc.querySelector('#init-btn');
        var time_btn = doc.querySelector('#time-btn');
        ...
        recover_btn.addEventListener('click', () => {
            ...
            jQuery.ajax({
                mathod: 'GET',
                url: 'http://' + HOST + ':3000/recover'
            });
            recover_btn.style.display = 'none'; 
            init_btn.style.display = 'block'; 
            time_btn.style.display = 'block'; 
        });
        ...
        init_btn.addEventListener('click', () => {
            ...
            recover_btn.style.display = 'block'; 
